...
int g_fMsnQuestion; // true if an idle grunt asked a question. Cleared when someone answers.
...
#define MASSN_DMG_HEADSHOT            ( DMG_BULLET | DMG_CLUB ) // damage types that can kill a grunt with a single headshot.
#define MASSN_NUM_HEADS               2                         // how many grunt heads are there?
...
public:
	...
	static const char *pMsnSentences[];
};

LINK_ENTITY_TO_CLASS( monster_male_assassin, CMassn );
LINK_ENTITY_TO_CLASS( monster_human_massn, CMassn );
...
const char *CMassn::pMsnSentences[] = {
...
{
	if ( m_iSentence == MASSN_SENT_NONE )
	{
		// no sentence cued up.
		return;
	}
	...
	if ( FOkToSpeak() )
	{
		SENTENCEG_PlayRndSz( ENT( pev ), pMsnSentences[m_iSentence], MASSN_SENTENCE_VOLUME, SFORCE_ATTN, 0, m_voicePitch );
		JustSpoke();
	}
}

//=========================================================
// IRelationship - overridden because Alien Msns are
// Human Msn's nemesis.
//=========================================================
...
//=========================================================
//=========================================================
void CMassn ::JustSpoke( void )
{
	CTalkMonster::g_talkWaitTime = gpGlobals->time + RANDOM_FLOAT( 1.5, 2.0 );
	m_iSentence = MASSN_SENT_NONE;
}
...
//=========================================================
// CheckRangeAttack1 - overridden for Massn, cause
// FCanCheckAttacks() doesn't disqualify all attacks based
// on whether or not the enemy is occluded because unlike
// the base class, the Massn can attack when the enemy is
...
//=========================================================
// CheckRangeAttack2 - this checks the Msn's grenade
// attack.
//=========================================================
...
{
	if ( FOkToSpeak() && ( g_fMsnQuestion || RANDOM_LONG( 0, 1 ) ) )
	{
		if ( !g_fMsnQuestion )
		{
			...
			{
			case 0: // check in
				SENTENCEG_PlayRndSz( ENT( pev ), "HG_CHECK", MASSN_SENTENCE_VOLUME, ATTN_NORM, 0, m_voicePitch );
				g_fMsnQuestion = 1;
				break;
			case 1: // question
				SENTENCEG_PlayRndSz( ENT( pev ), "HG_QUEST", MASSN_SENTENCE_VOLUME, ATTN_NORM, 0, m_voicePitch );
				g_fMsnQuestion = 2;
				break;
			case 2: // statement
				SENTENCEG_PlayRndSz( ENT( pev ), "HG_IDLE", MASSN_SENTENCE_VOLUME, ATTN_NORM, 0, m_voicePitch );
				break;
			}
		}
		else
		{
			switch ( g_fMsnQuestion )
			{
			case 1: // check in
				SENTENCEG_PlayRndSz( ENT( pev ), "HG_CLEAR", MASSN_SENTENCE_VOLUME, ATTN_NORM, 0, m_voicePitch );
				break;
			case 2: // question
				SENTENCEG_PlayRndSz( ENT( pev ), "HG_ANSWER", MASSN_SENTENCE_VOLUME, ATTN_NORM, 0, m_voicePitch );
				break;
			}
			g_fMsnQuestion = 0;
		}
...
void CMassn ::HandleAnimEvent( MonsterEvent_t *pEvent )
{
	...
	case MASSN_AE_BURST1:
	{
		...
		else
		{
			...
			EMIT_SOUND( ENT( pev ), CHAN_WEAPON, "weapons/m1014-1.wav", 1, ATTN_NORM );
		}
	...
	case MASSN_AE_CAUGHT_ENEMY:
	{
		if ( FOkToSpeak() )
		{
			SENTENCEG_PlayRndSz( ENT( pev ), "HG_ALERT", MASSN_SENTENCE_VOLUME, SFORCE_ATTN, 0, m_voicePitch );
			JustSpoke();
		}
	}
...
void CMassn ::Precache()
{
	...
	PRECACHE_SOUND( "weapons/m1014-1.wav" );
...
void CMassn ::PainSound( void )
{
	if ( gpGlobals->time > m_flNextPainTime )
	{
		...
		if ( RANDOM_LONG(0,99) < 5 )
		{
			...
			if (FOkToSpeak())
			{
				SENTENCEG_PlayRndSz(ENT(pev), "HG_PAIN", MASSN_SENTENCE_VOLUME, ATTN_NORM, 0, PITCH_NORM);
				JustSpoke();
				return;
			}
		}
...
Schedule_t slMsnCombatFail[] = {
	{ tlMsnCombatFail,
	  ARRAYSIZE( tlMsnCombatFail ),
...
Schedule_t slMsnVictoryDance[] = {
	{ tlMsnVictoryDance,
	  ARRAYSIZE( tlMsnVictoryDance ),
...
// the grunt to attack.
//=========================================================
Task_t tlMsnEstablishLineOfFire[] = {
...
Schedule_t slMsnEstablishLineOfFire[] = {
	{ tlMsnEstablishLineOfFire,
	  ARRAYSIZE( tlMsnEstablishLineOfFire ),
	  ...
	  bits_SOUND_DANGER,
	  "MsnEstablishLineOfFire" },
};

//=========================================================
// MsnFoundEnemy - grunt established sight with an enemy
// that was hiding from the squad.
//=========================================================
Task_t tlMsnFoundEnemy[] = {
	{ TASK_STOP_MOVING, 0 },
	{ TASK_FACE_ENEMY, (float)0 },
	{ TASK_PLAY_SEQUENCE_FACE_ENEMY, (float)ACT_SIGNAL1 },
};

Schedule_t slMsnFoundEnemy[] = {
	{ tlMsnFoundEnemy,
	  ARRAYSIZE( tlMsnFoundEnemy ),
...
//=========================================================
// MsnCombatFace Schedule
//=========================================================
Task_t tlMsnCombatFace1[] = {
	{ TASK_STOP_MOVING, 0 },
	{ TASK_SET_ACTIVITY, (float)ACT_IDLE },
	{ TASK_FACE_ENEMY, (float)0 },
	{ TASK_WAIT, (float)1.5 },
...
Schedule_t slMsnCombatFace[] = {
	{ tlMsnCombatFace1,
	  ARRAYSIZE( tlMsnCombatFace1 ),
...
Schedule_t slMsnSignalSuppress[] = {
	{ tlMsnSignalSuppress,
	  ARRAYSIZE( tlMsnSignalSuppress ),
...
Schedule_t slMsnSuppress[] = {
	{ tlMsnSuppress,
	  ARRAYSIZE( tlMsnSuppress ),
	  bits_COND_ENEMY_DEAD | bits_COND_LIGHT_DAMAGE | bits_COND_HEAVY_DAMAGE | bits_COND_HEAR_SOUND | bits_COND_SFORCE_NOFIRE | bits_COND_NO_AMMO_LOADED,
...
//=========================================================
Task_t tlMsnWaitInCover[] = {
	{ TASK_STOP_MOVING, (float)0 },
	{ TASK_SET_ACTIVITY, (float)ACT_IDLE },
	{ TASK_WAIT_FACE_ENEMY, (float)1 },
};

Schedule_t slMsnWaitInCover[] = {
	{ tlMsnWaitInCover,
	  ARRAYSIZE( tlMsnWaitInCover ),
...
Schedule_t slMsnTakeCover[] = {
	{ tlMsnTakeCover1,
	  ARRAYSIZE( tlMsnTakeCover1 ),
	  0,
	  0,
	  "TakeCover" },
};
...
Schedule_t slMsnGrenadeCover[] = {
	{ tlMsnGrenadeCover1,
	  ARRAYSIZE( tlMsnGrenadeCover1 ),
	  0,
	  0,
	  "GrenadeCover" },
};
...
// drop grenade then run to cover.
//=========================================================
Task_t tlMsnTossGrenadeCover1[] = {
	{ TASK_FACE_ENEMY, (float)0 },
	{ TASK_RANGE_ATTACK2, (float)0 },
	{ TASK_SET_SCHEDULE, (float)SCHED_TAKE_COVER_FROM_ENEMY },
};

Schedule_t slMsnTossGrenadeCover[] = {
	{ tlMsnTossGrenadeCover1,
	  ARRAYSIZE( tlMsnTossGrenadeCover1 ),
	  0,
	  0,
	  "TossGrenadeCover" },
};
...
//=========================================================
Task_t tlMsnTakeCoverFromBestSound[] = {
...
Schedule_t slMsnTakeCoverFromBestSound[] = {
	{ tlMsnTakeCoverFromBestSound,
	  ARRAYSIZE( tlMsnTakeCoverFromBestSound ),
	  0,
	  0,
	  "MsnTakeCoverFromBestSound" },
};

//=========================================================
// Msn reload schedule
//=========================================================
Task_t tlMsnHideReload[] = {
	{ TASK_STOP_MOVING, (float)0 },
...
Schedule_t slMsnHideReload[] = {
	{ tlMsnHideReload,
	  ARRAYSIZE( tlMsnHideReload ),
...
Schedule_t slMsnSweep[] = {
	{ tlMsnSweep,
	  ARRAYSIZE( tlMsnSweep ),
...
Schedule_t slMsnRangeAttack1A[] = {
	{ tlMsnRangeAttack1A,
	  ARRAYSIZE( tlMsnRangeAttack1A ),
...
Schedule_t slMsnRangeAttack1B[] = {
	{ tlMsnRangeAttack1B,
	  ARRAYSIZE( tlMsnRangeAttack1B ),
...
Schedule_t slMsnRangeAttack2[] = {
	{ tlMsnRangeAttack2,
	  ARRAYSIZE( tlMsnRangeAttack2 ),
	  0,
	  0,
	  "RangeAttack2" },
};
...
Schedule_t slMsnRepel[] = {
	{ tlMsnRepel,
	  ARRAYSIZE( tlMsnRepel ),
...
// repel
//=========================================================
Task_t tlMsnRepelAttack[] = {
	{ TASK_STOP_MOVING, (float)0 },
	{ TASK_FACE_ENEMY, (float)0 },
	{ TASK_PLAY_SEQUENCE, (float)ACT_FLY },
};

Schedule_t slMsnRepelAttack[] = {
	{ tlMsnRepelAttack,
	  ARRAYSIZE( tlMsnRepelAttack ),
...
Schedule_t slMsnRepelLand[] = {
	{ tlMsnRepelLand,
	  ARRAYSIZE( tlMsnRepelLand ),
	  bits_COND_SEE_ENEMY | bits_COND_NEW_ENEMY | bits_COND_LIGHT_DAMAGE | bits_COND_HEAVY_DAMAGE | bits_COND_HEAR_SOUND,
...
DEFINE_CUSTOM_SCHEDULES( CMassn ) {
	...
	slMsnTakeCover,
	slMsnGrenadeCover,
	slMsnTossGrenadeCover,
	slMsnTakeCoverFromBestSound,
	slMsnHideReload,
	slMsnSweep,
	slMsnRangeAttack1A,
	slMsnRangeAttack1B,
	...
	if ( HasConditions( bits_COND_HEAR_SOUND ) )
	{
		...
		if ( pSound )
		{
			if ( pSound->m_iType & bits_SOUND_DANGER )
			{
				...
				if ( FOkToSpeak() )
				{
					SENTENCEG_PlayRndSz( ENT( pev ), "HG_GREN", MASSN_SENTENCE_VOLUME, SFORCE_ATTN, 0, m_voicePitch );
					JustSpoke();
				}
	...
	case MONSTERSTATE_COMBAT:
	{
				...
				else
				{
					...
					if ( FOkToSpeak() ) // && RANDOM_LONG(0,1))
					{
						if ( ( m_hEnemy != NULL ) && m_hEnemy->IsPlayer() )
							...
							// monster
							SENTENCEG_PlayRndSz( ENT( pev ), "HG_MONST", MASSN_SENTENCE_VOLUME, SFORCE_ATTN, 0, m_voicePitch );
		...
		else if ( HasConditions( bits_COND_LIGHT_DAMAGE ) )
		{
			...
			if ( iPercent <= 90 && m_hEnemy != NULL )
			{
				...
				if ( FOkToSpeak() ) // && RANDOM_LONG(0,1))
				{
					//SENTENCEG_PlayRndSz( ENT(pev), "HG_COVER", MASSN_SENTENCE_VOLUME, SFORCE_ATTN, 0, m_voicePitch);
					m_iSentence = MASSN_SENT_COVER;
					//JustSpoke();
				}
				return GetScheduleOfType( SCHED_TAKE_COVER_FROM_ENEMY );
			}
		...
		else if ( HasConditions( bits_COND_CAN_RANGE_ATTACK1 ) )
		{
			...
			if ( OccupySlot( bits_SLOTS_MASSN_ENGAGE ) )
			{
				// try to take an available ENGAGE slot
				return GetScheduleOfType( SCHED_RANGE_ATTACK1 );
			}
			else if ( HasConditions( bits_COND_CAN_RANGE_ATTACK2 ) && OccupySlot( bits_SLOTS_MASSN_GRENADE ) )
			{
		...
		{
			if ( HasConditions( bits_COND_CAN_RANGE_ATTACK2 ) && OccupySlot( bits_SLOTS_MASSN_GRENADE ) )
			{
				//!!!KELLY - this grunt is about to throw or fire a grenade at the player. Great place for "fire in the hole"  "frag out" etc
				if ( FOkToSpeak() )
				{
					SENTENCEG_PlayRndSz( ENT( pev ), "HG_THROW", MASSN_SENTENCE_VOLUME, SFORCE_ATTN, 0, m_voicePitch );
					JustSpoke();
				}
				return GetScheduleOfType( SCHED_RANGE_ATTACK2 );
			}
			else if ( OccupySlot( bits_SLOTS_MASSN_ENGAGE ) )
			{
				//!!!KELLY - grunt cannot see the enemy and has just decided to
				// charge the enemy's position.
				if ( FOkToSpeak() ) // && RANDOM_LONG(0,1))
				{
					//SENTENCEG_PlayRndSz( ENT(pev), "HG_CHARGE", MASSN_SENTENCE_VOLUME, SFORCE_ATTN, 0, m_voicePitch);
					m_iSentence = MASSN_SENT_CHARGE;
					//JustSpoke();
				}
			...
			else
			{
				...
				if ( FOkToSpeak() && RANDOM_LONG( 0, 1 ) )
				{
					SENTENCEG_PlayRndSz( ENT( pev ), "HG_TAUNT", MASSN_SENTENCE_VOLUME, SFORCE_ATTN, 0, m_voicePitch );
					JustSpoke();
				}
...
Schedule_t *CMassn ::GetScheduleOfType( int Type )
{
	...
	case SCHED_TAKE_COVER_FROM_ENEMY:
	{
		if ( InSquad() )
		{
			if ( g_iSkillLevel == SKILL_HARD && HasConditions( bits_COND_CAN_RANGE_ATTACK2 ) && OccupySlot( bits_SLOTS_MASSN_GRENADE ) )
			{
				if ( FOkToSpeak() )
				{
					SENTENCEG_PlayRndSz( ENT( pev ), "HG_THROW", MASSN_SENTENCE_VOLUME, SFORCE_ATTN, 0, m_voicePitch );
				...
				return slMsnTossGrenadeCover;
			}
			else
			{
				return &slMsnTakeCover[0];
			}
		}
		else
		{
			...
			{
				return &slMsnGrenadeCover[0];
			}
		}
	}
	case SCHED_TAKE_COVER_FROM_BEST_SOUND:
	{
		return &slMsnTakeCoverFromBestSound[0];
	}
	case SCHED_SFORCE_TAKECOVER_FAILED:
	{
		if ( HasConditions( bits_COND_CAN_RANGE_ATTACK1 ) && OccupySlot( bits_SLOTS_MASSN_ENGAGE ) )
		{
			return GetScheduleOfType( SCHED_RANGE_ATTACK1 );
		}
	...
	{
		return &slMsnEstablishLineOfFire[0];
	}
	break;
	case SCHED_RANGE_ATTACK1:
	{
		...
		if ( m_fStanding )
			return &slMsnRangeAttack1B[0];
		else
			return &slMsnRangeAttack1A[0];
	}
	case SCHED_RANGE_ATTACK2:
	{
		return &slMsnRangeAttack2[0];
	}
	case SCHED_COMBAT_FACE:
	{
		return &slMsnCombatFace[0];
	}
	case SCHED_SFORCE_WAIT_FACE_ENEMY:
	{
		return &slMsnWaitInCover[0];
	}
	...
	{
		return &slMsnHideReload[0];
	}
	case SCHED_SFORCE_FOUND_ENEMY:
	{
		return &slMsnFoundEnemy[0];
	}
	...
	case SCHED_SFORCE_SUPPRESS:
	{
		if ( m_hEnemy->IsPlayer() && m_fFirstEncounter )
		{
			...
			return &slMsnSignalSuppress[0];
		}
		else
		{
			return &slMsnSuppress[0];
		}
	}
	case SCHED_FAIL:
	{
	...
	case SCHED_SFORCE_REPEL_ATTACK:
	{
		if ( pev->velocity.z > -128 )
			pev->velocity.z -= 32;
		return &slMsnRepelAttack[0];
	}
	case SCHED_SFORCE_REPEL_LAND:
	{
		return &slMsnRepelLand[0];
	}
	default:
	{
...
//=========================================================
// CMassnRepel - when triggered, spawns a monster_massn
...
LINK_ENTITY_TO_CLASS( monster_male_assassin_repel, CMassnRepel );
...
void CMassnRepel::RepelUse( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	...
	*/
	...
	CBaseMonster *pMsn = pEntity->MyMonsterPointer();
	pMsn->pev->movetype = MOVETYPE_FLY;
	pMsn->pev->velocity = Vector( 0, 0, RANDOM_FLOAT( -196, -128 ) );
	pMsn->SetActivity( ACT_GLIDE );
	// UNDONE: position?
	pMsn->m_vecLastPosition = tr.vecEndPos;

	CBeam *pBeam = CBeam::BeamCreate( "sprites/rope.spr", 10 );
	pBeam->PointEntInit( pev->origin + Vector( 0, 0, 112 ), pMsn->entindex() );
	pBeam->SetFlags( BEAM_FSOLID );
	pBeam->SetColor( 255, 255, 255 );
	pBeam->SetThink( &CBaseEntity::SUB_Remove );
	pBeam->pev->nextthink = gpGlobals->time + -4096.0 * tr.flFraction / pMsn->pev->velocity.z + 0.5;
...
char *CDeadMassn::m_szPoses[] = { "deadstomach", "deadside", "deadsitting" };
...
LINK_ENTITY_TO_CLASS( monster_male_assassin_dead, CMassn );

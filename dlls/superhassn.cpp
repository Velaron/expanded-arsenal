...
//=========================================================
// super - Human super, fast and stealthy
//=========================================================
...
enum
{
	SCHED_SUPER_EXPOSED = LAST_COMMON_SCHEDULE + 1, // cover was blown.
	SCHED_SUPER_JUMP,                               // fly through the air
	SCHED_SUPER_JUMP_ATTACK,                        // fly through the air and shoot
	SCHED_SUPER_JUMP_LAND,                          // hit and run away
};
...
enum
{
	TASK_SUPER_FALL_TO_GROUND = LAST_COMMON_TASK + 1, // falling and waiting to hit ground
};

//=========================================================
// Monster's Anim Events Go Here
//=========================================================
#define SUPER_AE_SHOOT1    1
#define SUPER_AE_TOSS1     2
...
class CSuper : public CBaseMonster
{
public:
	void Spawn( void );
	void Precache( void );
...
LINK_ENTITY_TO_CLASS( monster_human_super, CSuper );

TYPEDESCRIPTION CSuper::m_SaveData[] = {
	DEFINE_FIELD( CSuper, m_flLastShot, FIELD_TIME ),
	DEFINE_FIELD( CSuper, m_flDiviation, FIELD_FLOAT ),

	DEFINE_FIELD( CSuper, m_flNextJump, FIELD_TIME ),
	DEFINE_FIELD( CSuper, m_vecJumpVelocity, FIELD_VECTOR ),

	DEFINE_FIELD( CSuper, m_flNextGrenadeCheck, FIELD_TIME ),
	DEFINE_FIELD( CSuper, m_vecTossVelocity, FIELD_VECTOR ),
	DEFINE_FIELD( CSuper, m_fThrowGrenade, FIELD_BOOLEAN ),

	DEFINE_FIELD( CSuper, m_iTargetRanderamt, FIELD_INTEGER ),
	DEFINE_FIELD( CSuper, m_iFrustration, FIELD_INTEGER ),
};

IMPLEMENT_SAVERESTORE( CSuper, CBaseMonster );

//=========================================================
// DieSound
//=========================================================
void CSuper ::GibMonster( void )
{
	if ( GetBodygroup( 1 ) != 1 )
	{ // drop the gun!
		Vector vecGunPos;
...
void CSuper ::DeathSound( void )
{
	if ( pev->body < SUPER_BODY_GUNGONE )
	{ // drop the gun!
		Vector vecGunPos;
		Vector vecGunAngles;
...
void CSuper ::Shoot( void )
{
	...
	FireBullets( 1, vecShootOrigin, vecShootDir, Vector( m_flDiviation, m_flDiviation, m_flDiviation ), 2048, BULLET_MONSTER_357 ); // shoot +-8 degrees

	switch ( RANDOM_LONG( 0, 1 ) )
	{
	case 0:
		EMIT_SOUND( ENT( pev ), CHAN_WEAPON, "weapons/pl_gun4.wav", RANDOM_FLOAT( 0.9, 1.0 ), ATTN_NORM );
		break;
	case 1:
		EMIT_SOUND( ENT( pev ), CHAN_WEAPON, "weapons/pl_gun4.wav", RANDOM_FLOAT( 0.9, 1.0 ), ATTN_NORM );
		break;
...
//=========================================================
void CSuper ::HandleAnimEvent( MonsterEvent_t *pEvent )
{
	Vector vecGunPos;
	Vector vecGunAngles;
...
void CSuper ::Spawn()
{
	...
	SET_MODEL( ENT( pev ), "models/super.mdl" );
	...
	pev->effects = 0;
	pev->health = gSkillData.superHealth;
...
//=========================================================
void CSuper ::Precache()
{
	PRECACHE_MODEL( "models/super.mdl" );

	PRECACHE_SOUND( "weapons/pl_gun4.wav" );
...
Schedule_t slSuperFail[] = {
	{ tlSuperFail,
	  ARRAYSIZE( tlSuperFail ),
	  bits_COND_LIGHT_DAMAGE | bits_COND_HEAVY_DAMAGE | bits_COND_PROVOKED | bits_COND_CAN_RANGE_ATTACK1 | bits_COND_CAN_RANGE_ATTACK2 | bits_COND_CAN_MELEE_ATTACK1 | bits_COND_HEAR_SOUND,
...
//=========================================================
Task_t tlSuperExposed[] = {
	{ TASK_STOP_MOVING, (float)0 },
	{ TASK_RANGE_ATTACK1, (float)0 },
	{ TASK_SET_FAIL_SCHEDULE, (float)SCHED_SUPER_JUMP },
	{ TASK_SET_SCHEDULE, (float)SCHED_TAKE_COVER_FROM_ENEMY },
};

Schedule_t slSuperExposed[] = {
	{
	    tlSuperExposed,
	    ARRAYSIZE( tlSuperExposed ),
...
// cover!
//=========================================================
Task_t tlSuperTakeCoverFromEnemy[] = {
	{ TASK_STOP_MOVING, (float)0 },
	{ TASK_WAIT, (float)0.2 },
...
Schedule_t slSuperTakeCoverFromEnemy[] = {
	{ tlSuperTakeCoverFromEnemy,
	  ARRAYSIZE( tlSuperTakeCoverFromEnemy ),
	  bits_COND_NEW_ENEMY | bits_COND_CAN_MELEE_ATTACK1 | bits_COND_HEAR_SOUND,

	  bits_SOUND_DANGER,
	  "SuperTakeCoverFromEnemy" },
};
...
// cover!
//=========================================================
Task_t tlSuperTakeCoverFromEnemy2[] = {
	{ TASK_STOP_MOVING, (float)0 },
	{ TASK_WAIT, (float)0.2 },
	{ TASK_FACE_ENEMY, (float)0 },
	{ TASK_RANGE_ATTACK1, (float)0 },
...
Schedule_t slSuperTakeCoverFromEnemy2[] = {
	{ tlSuperTakeCoverFromEnemy2,
	  ARRAYSIZE( tlSuperTakeCoverFromEnemy2 ),
	  bits_COND_NEW_ENEMY | bits_COND_CAN_MELEE_ATTACK2 | bits_COND_HEAR_SOUND,

	  bits_SOUND_DANGER,
	  "SuperTakeCoverFromEnemy2" },
};

//=========================================================
// hide from the loudest sound source
//=========================================================
Task_t tlSuperTakeCoverFromBestSound[] = {
...
Schedule_t slSuperTakeCoverFromBestSound[] = {
	{ tlSuperTakeCoverFromBestSound,
	  ARRAYSIZE( tlSuperTakeCoverFromBestSound ),
	  bits_COND_NEW_ENEMY,
	  0,
	  "SuperTakeCoverFromBestSound" },
};
...
Schedule_t slSuperHide[] = {
	{ tlSuperHide,
	  ARRAYSIZE( tlSuperHide ),
	  bits_COND_NEW_ENEMY | bits_COND_SEE_ENEMY | bits_COND_SEE_FEAR | bits_COND_LIGHT_DAMAGE | bits_COND_HEAVY_DAMAGE | bits_COND_PROVOKED | bits_COND_HEAR_SOUND,
...
Schedule_t slSuperHunt[] = {
	{ tlSuperHunt,
	  ARRAYSIZE( tlSuperHunt ),
	  bits_COND_NEW_ENEMY |
	      // bits_COND_SEE_ENEMY			|
...
Task_t tlSuperJump[] = {
	...
	{ TASK_SET_SCHEDULE, (float)SCHED_SUPER_JUMP_ATTACK },
};

Schedule_t slSuperJump[] = {
	{ tlSuperJump,
	  ARRAYSIZE( tlSuperJump ),
	  0,
	  0,
	  "SuperJump" },
};
...
// repel
//=========================================================
Task_t tlSuperJumpAttack[] = {
	{ TASK_SET_FAIL_SCHEDULE, (float)SCHED_SUPER_JUMP_LAND },
	// { TASK_SET_ACTIVITY,		(float)ACT_FLY	},
	{ TASK_SUPER_FALL_TO_GROUND, (float)0 },
};

Schedule_t slSuperJumpAttack[] = {
	{ tlSuperJumpAttack,
	  ARRAYSIZE( tlSuperJumpAttack ),
	  0,
	  0,
	  "SuperJumpAttack" },
};
...
// repel
//=========================================================
Task_t tlSuperJumpLand[] = {
	{ TASK_SET_FAIL_SCHEDULE, (float)SCHED_SUPER_EXPOSED },
	// { TASK_SET_FAIL_SCHEDULE,		(float)SCHED_MELEE_ATTACK1	},
	{ TASK_SET_ACTIVITY, (float)ACT_IDLE },
...
Schedule_t slSuperJumpLand[] = {
	{ tlSuperJumpLand,
	  ARRAYSIZE( tlSuperJumpLand ),
	  0,
	  0,
	  "SuperJumpLand" },
};

DEFINE_CUSTOM_SCHEDULES( CSuper ) {
	slSuperFail,
	slSuperExposed,
	slSuperTakeCoverFromEnemy,
	slSuperTakeCoverFromEnemy2,
	slSuperTakeCoverFromBestSound,
	slSuperHide,
	slSuperHunt,
	slSuperJump,
	slSuperJumpAttack,
...
IMPLEMENT_CUSTOM_SCHEDULES( CSuper, CBaseMonster );
...
//
//=========================================================
BOOL CSuper ::CheckRangeAttack1( float flDot, float flDist )
{
...
//=========================================================
BOOL CSuper ::CheckRangeAttack2( float flDot, float flDist )
{
	m_fThrowGrenade = FALSE;
...
// StartTask
//=========================================================
void CSuper ::StartTask( Task_t *pTask )
{
	switch ( pTask->iTask )
	{
	case TASK_RANGE_ATTACK2:
		...
		break;
	case TASK_SUPER_FALL_TO_GROUND:
		break;
	default:
		CBaseMonster ::StartTask( pTask );
		break;
...
// RunTask
//=========================================================
void CSuper ::RunTask( Task_t *pTask )
{
	switch ( pTask->iTask )
	{
	case TASK_SUPER_FALL_TO_GROUND:
...
// of the proper type.
//=========================================================
Schedule_t *CSuper ::GetSchedule( void )
{
	...
	case MONSTERSTATE_COMBAT:
	{
		...
		if ( pev->movetype == MOVETYPE_TOSS )
		{
			if ( pev->flags & FL_ONGROUND )
			{
				...
				// just landed
				pev->movetype = MOVETYPE_STEP;
				return GetScheduleOfType( SCHED_SUPER_JUMP_LAND );
			}
			else
			{
				...
				if ( m_MonsterState == MONSTERSTATE_COMBAT )
					return GetScheduleOfType( SCHED_SUPER_JUMP );
				else
					return GetScheduleOfType( SCHED_SUPER_JUMP_ATTACK );
			}
		}
		...
		if ( HasConditions( bits_COND_SEE_ENEMY ) && HasConditions( bits_COND_ENEMY_FACING_ME ) )
		{
			...
			m_iFrustration++;
			return GetScheduleOfType( SCHED_SUPER_EXPOSED );
		}
...
//=========================================================
//=========================================================
Schedule_t *CSuper ::GetScheduleOfType( int Type )
{
	// ALERT( at_console, "%d\n", m_iFrustration );
	switch ( Type )
	{
	case SCHED_TAKE_COVER_FROM_ENEMY:
		if ( pev->health > 30 )
			return slSuperTakeCoverFromEnemy;
		else
			return slSuperTakeCoverFromEnemy2;
	case SCHED_TAKE_COVER_FROM_BEST_SOUND:
		return slSuperTakeCoverFromBestSound;
	case SCHED_SUPER_EXPOSED:
		return slSuperExposed;
	case SCHED_FAIL:
		if ( m_MonsterState == MONSTERSTATE_COMBAT )
			return slSuperFail;
		break;
	...
	case SCHED_MELEE_ATTACK1:
		...
		{
			return slSuperJumpAttack;
		}
	case SCHED_SUPER_JUMP:
	case SCHED_SUPER_JUMP_ATTACK:
		return slSuperJumpAttack;
	case SCHED_SUPER_JUMP_LAND:
		return slSuperJumpLand;
	}

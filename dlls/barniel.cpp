...
Schedule_t slBlFollow[] = {
	{ tlBlFollow,
	  ARRAYSIZE( tlBlFollow ),
...
//=========================================================
Task_t tlBlrnielEnemyDraw[] = {
	{ TASK_STOP_MOVING, 0 },
	{ TASK_FACE_ENEMY, 0 },
	{ TASK_PLAY_SEQUENCE_FACE_ENEMY, (float)ACT_ARM },
};

Schedule_t slBlrnielEnemyDraw[] = {
	{ tlBlrnielEnemyDraw,
	  ARRAYSIZE( tlBlrnielEnemyDraw ),
	  0,
	  0,
	  "Barniel Enemy Draw" }
};

Task_t tlBlFaceTarget[] = {
...
Schedule_t slBlFaceTarget[] = {
	{ tlBlFaceTarget,
	  ARRAYSIZE( tlBlFaceTarget ),
...
Schedule_t slIdleBlStand[] = {
	{ tlIdleBlStand,
	  ARRAYSIZE( tlIdleBlStand ),
...
void CBarniel ::BarnielFirePistol( void )
{
	...
	FireBullets( 1, vecShootOrigin, vecShootDir, VECTOR_CONE_2DEGREES, 1024, BULLET_MONSTER_P904 );
	...
	if ( pitchShift > 10 )
		...
		pitchShift -= 5;
	EMIT_SOUND_DYN( ENT( pev ), CHAN_WEAPON, "barniel/ba_attack2.wav", 1, ATTN_NORM, 0, 100 + pitchShift );
...
void CBarniel ::Precache()
{
	...
	PRECACHE_SOUND( "barniel/ba_attack1.wav" );
	PRECACHE_SOUND( "barniel/ba_attack2.wav" );

	PRECACHE_SOUND( "barniel/ba_pain1.wav" );
	PRECACHE_SOUND( "barniel/ba_pain2.wav" );
	PRECACHE_SOUND( "barniel/ba_pain3.wav" );

	PRECACHE_SOUND( "barniel/ba_die1.wav" );
	PRECACHE_SOUND( "barniel/ba_die2.wav" );
	PRECACHE_SOUND( "barniel/ba_die3.wav" );
...
void CBarniel ::PainSound( void )
{
	...
	{
	case 0: EMIT_SOUND_DYN( ENT( pev ), CHAN_VOICE, "barniel/ba_pain1.wav", 1, ATTN_NORM, 0, GetVoicePitch() ); break;
	case 1: EMIT_SOUND_DYN( ENT( pev ), CHAN_VOICE, "barniel/ba_pain2.wav", 1, ATTN_NORM, 0, GetVoicePitch() ); break;
	case 2: EMIT_SOUND_DYN( ENT( pev ), CHAN_VOICE, "barniel/ba_pain3.wav", 1, ATTN_NORM, 0, GetVoicePitch() ); break;
	}
}
...
void CBarniel ::DeathSound( void )
{
	switch ( RANDOM_LONG( 0, 2 ) )
	{
	case 0: EMIT_SOUND_DYN( ENT( pev ), CHAN_VOICE, "barniel/ba_die1.wav", 1, ATTN_NORM, 0, GetVoicePitch() ); break;
	case 1: EMIT_SOUND_DYN( ENT( pev ), CHAN_VOICE, "barniel/ba_die2.wav", 1, ATTN_NORM, 0, GetVoicePitch() ); break;
	case 2: EMIT_SOUND_DYN( ENT( pev ), CHAN_VOICE, "barniel/ba_die3.wav", 1, ATTN_NORM, 0, GetVoicePitch() ); break;
	}
}
...
void CBarniel::Killed( entvars_t *pevAttacker, int iGib )
{
	...
	{ // drop the gun!
		...
		CBaseEntity *pGun = DropItem( "weapon_p904", vecGunPos, vecGunAngles );
	}
...
Schedule_t *CBarniel ::GetScheduleOfType( int Type )
{
	...
	case SCHED_TARGET_FACE:
		...
		if ( psched == slIdleStand )
			return slBlFaceTarget; // override this for different target face behavior
		else
			return psched;
